...
export const AsciiHero = () => {
    ...
    const draw = () => {
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < columns; x++) {
          ...
          const noise =
            Math.sin(x * 0.1 + time) * Math.cos(y * 0.1 + time) * 0.2;
          ...
          if (isHeart) {
            ...
            const charIndex =
              Math.floor((Math.sin(x * y + time * 2) + 1) * 2.5) % chars.length;
  ...
  return (
    <section className="relative w-full min-h-screen bg-black text-white overflow-hidden flex items-center justify-center">
      ...
      <div
        className="absolute inset-0 bg-radial-gradient from-transparent via-black/50 to-black pointer-events-none"
        style={{
          background:
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center flex flex-col items-center">
        ...
        >
          <span className="w-1 h-1 bg-emerald-500 rounded-none" />{" "}
          {/* Sharp dot */}
          Introducing Aevio
        </motion.div>
        ...
        >
          Track workouts, journal your mind, and analyze your heart health —
          powered by AI.
        </motion.p>
